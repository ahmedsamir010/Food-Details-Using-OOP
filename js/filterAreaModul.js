...
export class filterArea {
    ...
    async finalFilterArea(meal) {
      let filterArea = await this.getApi(meal);
      let cartonaFilterArea = "";
      ...
      for (let i = 0; i < filterArea.length; i++) {
        ...
        <img
          src="${filterArea[i].strMealThumb}"
          alt=""
          class="w-100 rounded-2"
        />
        <div class="imgLayer d-flex align-items-center">
          <h2 class="p-2">${filterArea[i].strMeal}</h2>
        </div>
        </div>
      </div>
  ...
  document.getElementById("sectionFilterArea").classList.remove("d-none")
      ...
      document.getElementById("displayFilterArea").innerHTML = cartonaFilterArea;
